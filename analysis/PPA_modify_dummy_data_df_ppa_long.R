...
eligible_persons <- df_ppa_long %>%
  distinct(patient_id, date, variable, instance, cens_date_antidiab_start_cont) %>%
  filter(!is.na(date) & !is.na(cens_date_antidiab_start_cont)) %>%
  pull(patient_id)
...
new_dates <- df_ppa_long %>%
  ...
  mutate(
    new_date = cens_date_antidiab_start_cont + sample(c(-3:-1, 1:3), n(), replace=TRUE)
  ) %>%
