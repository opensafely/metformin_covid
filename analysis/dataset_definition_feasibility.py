...
studystartlandmark_date = study_dates["studystartlandmark_date"]
...
#######################################################################################
# First METFORMIN prescription, after studystartlandmark_date, based on codelist https://www.opencodelists.org/codelist/user/john-tazare/metformin-dmd/48e43356/
dataset.exp_date_metfin_first = first_matching_med_dmd_between(metformin_codes_dmd, studystartlandmark_date, feasibilityend_date).date 
dataset.exp_count_metfin = (
  medications.where(
    medications.dmd_code.is_in(metformin_codes_dmd))
    .where(medications.date.is_on_or_between(studystartlandmark_date, feasibilityend_date))
    .count_for_patient()
)
...
dataset.elig_date_t2dm_original = minimum_of(
    first_matching_event_clinical_ctv3_between(diabetes_type2_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date,
    first_matching_event_apc_between(diabetes_type2_icd10, studystartlandmark_date, studystartlandmark_date + days(366)).admission_date
) 
...
dataset.elig_date_t1dm_original = minimum_of(
    first_matching_event_clinical_ctv3_between(diabetes_type1_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date,
    first_matching_event_apc_between(diabetes_type1_icd10, studystartlandmark_date, studystartlandmark_date + days(366)).admission_date
) 
...
dataset.elig_date_gestationaldm_original = minimum_of(
    first_matching_event_clinical_ctv3_between(diabetes_gestational_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date,
    first_matching_event_apc_between(gestationaldm_icd10, studystartlandmark_date, studystartlandmark_date + days(366)).admission_date
) 
## PCOS
dataset.elig_date_pcos = minimum_of(
    first_matching_event_clinical_snomed_between(pcos_snomed_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date,
    first_matching_event_apc_between(pcos_icd10, studystartlandmark_date, studystartlandmark_date + days(366)).admission_date
)
...
dataset.tmp_cov_date_t1dm_ctv3 = first_matching_event_clinical_ctv3_between(diabetes_type1_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date
dataset.cov_date_t1dm = minimum_of(
    (first_matching_event_clinical_ctv3_between(diabetes_type1_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date),
    (first_matching_event_apc_between(diabetes_type1_icd10, studystartlandmark_date, studystartlandmark_date + days(366)).admission_date)
)
# Count codes (individually and together, for diabetes algo)
dataset.tmp_cov_count_t1dm_ctv3 = count_matching_event_clinical_ctv3_between(diabetes_type1_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366))
dataset.tmp_cov_count_t1dm_hes = count_matching_event_apc_between(diabetes_type1_icd10, studystartlandmark_date, studystartlandmark_date + days(366))
dataset.tmp_cov_count_t1dm = dataset.tmp_cov_count_t1dm_ctv3 + dataset.tmp_cov_count_t1dm_hes

## Type 2 Diabetes
# First date from primary+secondary, but also primary care date separately for diabetes algo)
dataset.tmp_cov_date_t2dm_ctv3 = first_matching_event_clinical_ctv3_between(diabetes_type2_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date
dataset.cov_date_t2dm = minimum_of(
    (first_matching_event_clinical_ctv3_between(diabetes_type2_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date),
    (first_matching_event_apc_between(diabetes_type2_icd10, studystartlandmark_date, studystartlandmark_date + days(366)).admission_date)
)
# Count codes (individually and together, for diabetes algo)
dataset.tmp_cov_count_t2dm_ctv3 = count_matching_event_clinical_ctv3_between(diabetes_type2_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366))
dataset.tmp_cov_count_t2dm_hes = count_matching_event_apc_between(diabetes_type2_icd10, studystartlandmark_date, studystartlandmark_date + days(366))
dataset.tmp_cov_count_t2dm = dataset.tmp_cov_count_t2dm_ctv3 + dataset.tmp_cov_count_t2dm_hes

## Diabetes unspecified/other
# First date
dataset.cov_date_otherdm = first_matching_event_clinical_ctv3_between(diabetes_other_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date
# Count codes
dataset.tmp_cov_count_otherdm = count_matching_event_clinical_ctv3_between(diabetes_other_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366))

## Gestational diabetes
# First date
dataset.cov_date_gestationaldm = first_matching_event_clinical_ctv3_between(diabetes_gestational_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date

## Diabetes diagnostic codes
# First date
dataset.tmp_cov_date_poccdm = first_matching_event_clinical_ctv3_between(diabetes_diagnostic_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date
# Count codes
dataset.tmp_cov_count_poccdm_ctv3 = count_matching_event_clinical_ctv3_between(diabetes_diagnostic_ctv3_clinical, studystartlandmark_date, studystartlandmark_date + days(366))
...
dataset.tmp_cov_num_max_hba1c_mmol_mol = (
  clinical_events.where(
    clinical_events.snomedct_code.is_in(hba1c_snomed))
    .where(clinical_events.date.is_on_or_between(studystartlandmark_date, studystartlandmark_date + days(366)))
...
dataset.tmp_cov_date_max_hba1c = ( 
  clinical_events.where(
    clinical_events.snomedct_code.is_in(hba1c_snomed))
    .where(clinical_events.date.is_on_or_between(studystartlandmark_date, studystartlandmark_date + days(366))) # this line of code probably not needed again
...
## Diabetes drugs
# First dates
dataset.tmp_cov_date_insulin_snomed = first_matching_med_dmd_between(insulin_snomed_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date
dataset.tmp_cov_date_antidiabetic_drugs_snomed = first_matching_med_dmd_between(antidiabetic_drugs_snomed_clinical, studystartlandmark_date, studystartlandmark_date + days(366)).date
dataset.tmp_cov_date_nonmetform_drugs_snomed = first_matching_med_dmd_between(non_metformin_dmd, studystartlandmark_date, studystartlandmark_date + days(366)).date # this extra step makes sense for the diabetes algorithm (otherwise not)
