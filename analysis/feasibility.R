...
## 2. How many with a T2DM diagnosis start metformin thereafter (in monthly periods until max 6m) and median time to start?
# Use T2DM variable from diabetes algo
...
## 3. How many with a T2DM diagnosis start metformin thereafter (in monthly periods til max 6m) and median time to start AND have a covid event?
# Use T2DM variable from diabetes algo
periods <- c(30, 60, 90, 120, 150, 180) # in days - but now, we are interested in e.g. monthly until max. 6-monthly intervals

## function with midpoint6 rounding
fn_t2dm_metfin_start_covid_midpoint6 <- function(periods){
  data_processed %>%
    filter(!is.na(cov_date_t2dm)) %>%
    filter(!is.na(exp_date_metfin_first)) %>%
    filter(exp_date_metfin_first > cov_date_t2dm) %>%
    filter(exp_date_metfin_first <= (cov_date_t2dm + days(periods))) %>%
    filter(cov_date_covid19_first > exp_date_metfin_first) %>% # and had a covid event after metformin start
...
n_t2dm_metfin_start_covid_midpoint6 <- map_dfr(periods, fn_t2dm_metfin_start_covid_midpoint6)

## function without midpoint6 rounding
fn_t2dm_metfin_start_covid <- function(periods){
  data_processed %>%
    filter(!is.na(cov_date_t2dm)) %>%
    filter(!is.na(exp_date_metfin_first)) %>%
    filter(exp_date_metfin_first > cov_date_t2dm) %>%
...
n_t2dm_metfin_start_covid <- map_dfr(periods, fn_t2dm_metfin_start_covid)
...
write.csv(n_t2dm_metfin_start_covid_midpoint6, file = here::here("output", "data_properties", "n_t2dm_metfin_start_covid_midpoint6.csv"))
write.csv(n_t2dm_metfin_start_covid, file = here::here("output", "data_properties", "n_t2dm_metfin_start_covid.csv"))
