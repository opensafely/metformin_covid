...
covariate_names <- names(df_months_severecovid) %>%
  ...
  setdiff(c("cov_num_bmi",
            ...
            "cov_bin_carehome_status" # will be taken out in new dataset version
            )) 
# print(covariate_names)
# summary(df_months_severecovid$cov_cat_tc_hdl_ratio)
# summary(df_months_severecovid$cov_cat_bmi)
# summary(df_months_severecovid$cov_cat_hba1c_mmol_mol)
...
if (Sys.getenv("OPENSAFELY_BACKEND") %in% c("", "expectations")) {
  ...
  source("analysis/modify_dummy_data_df_months_severecovid.R")
...
df_base <- df_months_severecovid %>%
  filter(month == 0) %>%
  mutate(iptw_denom = predict(iptw.denom, ., type = "response"))
df_months_severecovid <- df_months_severecovid %>%
  left_join(df_base %>% select(patient_id, iptw_denom), by = "patient_id")

# Estimate stabilized weights
df_months_severecovid <- df_months_severecovid %>%
  mutate(w_treat_stab = ifelse(exp_bin_treat == 1,
                               mean(exp_bin_treat) / iptw_denom,
                               (1 - mean(exp_bin_treat)) / (1 - iptw_denom)))
...
ipcw_denom_formula <- as.formula(paste("cens_bin_metfin_start_cont == 0 ~ month + monthsqr +", paste(covariate_names, collapse = " + ")))
...
ipcw_num_formula <- as.formula(paste("cens_bin_metfin_start_cont == 0 ~ month + monthsqr"))
...
#   select(patient_id, outcome, exp_bin_treat, cens_bin_metfin_start_cont, month, w_treat_cens_stab_trunc, w_treat_stab, w_cens_stab, ipcw_num, ipcw_denom) %>%
#   # filter(cens_bin_metfin_start_cont == 1) %>%
#   # filter(w_treat_cens_stab_trunc > 3000) %>%
#   View()


# IPTW & IPCW for PPA: Check the summary weights and the balance ---------------------------------------
print('IPTW & IPCW for PPA: Check the weights and the balance')
...
# but include exp_bin_treat here (different to PPA!)
ipcw_ltfu_denom_formula <- as.formula(paste("censor_LTFU == 0 ~ exp_bin_treat + month + monthsqr +", paste(covariate_names, collapse = " + ")))
...
## For the numerator we now fit a model to include the time-varying aspect of the weights - but without any covariates
ipcw_ltfu_num_formula <- as.formula(paste("censor_LTFU == 0 ~ month + monthsqr"))
...
# IPTW & IPCW for PPA & IPCW for LTFU: Check the summary weights and the balance ---------------------------------------
print('IPTW & IPCW for PPA & IPCW for LTFU: Check the weights and the balance')

# Create subsets of data, according to treat_b status
treat_b0 <- subset(df_months_severecovid, exp_bin_treat==0)
treat_b1 <- subset(df_months_severecovid, exp_bin_treat==1)
w_treat_0 <- treat_b0$w_treat_cens_ltfu_stab_trunc
w_treat_1 <- treat_b1$w_treat_cens_ltfu_stab_trunc
